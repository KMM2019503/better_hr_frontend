  ...
  <div class="common-layout">
    <el-container>
      ...
      <el-main class="rounded-tl-3xl rounded-bl-3xl shadow-2xl bg-white !p-0">
        ...
        >
          <div class="relative items-center block max-w-sm p-6 bg-transparent">
            ...
            >
              <svg
                aria-hidden="true"
                class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-[#1ed292]"
                viewBox="0 0 100 101"
                fill="none"
        ...
        <!-- <client-only>
          <div
            v-if="status._value !== 'success'"
            class="w-full h-full flex justify-center items-center"
          >
            <div
              class="relative items-center block max-w-sm p-6 bg-transparent"
            >
              <div
                role="status"
                class="absolute -translate-x-1/2 -translate-y-1/2 top-2/4 left-1/2"
...
const fetchEmployees = async () => {
  ...
  try {
    employeesData.value = await employeeStore.getEmployeesData(1, 20);
  } catch (error) {
...
// console.log("🚀 ~ status:", status._value);
</script>
